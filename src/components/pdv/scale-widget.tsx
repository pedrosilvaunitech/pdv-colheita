...
export function ScaleWidget({ onWeight }: { onWeight?: (kg: number) => void }) {
  ...
  useEffect(() => {
    ...
    getScaleStatus()
      .then((st) => {
        if (alive) setAgentScale(!!st.available && !!st.config.enabled);
      })
      .catch(() => {
        if (alive) setAgentScale(false);
      });
    return () => {
      alive = false;
    };
  ...
  const doRead = async () => {
    ...
    try {
      ...
      if (r.status === "overload") {
        toast.error("Balança em sobrecarga");
        return;
      }
      if (r.status === "unstable") {
        toast.warning("Peso instável — aguarde estabilizar");
        return;
      }
      if (r.weightKg <= 0) {
        toast.error("Peso zero na balança");
        return;
      }
      onWeight?.(r.weightKg);
    ...
    <div className="flex items-center gap-1">
      {connected || agentScale ? (
        ...
        <Button
          ...
          size="sm"
          onClick={connect}
          title="Conectar à porta serial da balança Toledo"
        >
      ...
      <Dialog open={cfgOpen} onOpenChange={setCfgOpen}>
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Balança Toledo</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              ...
              <Select
                ...
                onValueChange={(v) => updateConfig({ protocol: v as ToledoProtocol })}
              >
                ...
                <SelectContent>
                  ...
                  <SelectItem value="prix4-p1">
                    Toledo Prix 4/5 · Protocolo 1 (estendido)
                  </SelectItem>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <Select
                  value={config.parity}
                  onValueChange={(v) => updateConfig({ parity: v as ParityType })}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => updateConfig({ dataBits: Number(v) as 7 | 8 })}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => updateConfig({ stopBits: Number(v) as 1 | 2 })}
                >
                  <SelectTrigger className="mt-1">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  updateConfig({ requestTimeoutMs: Math.max(200, Number(e.target.value) || 1500) })
                }
              />
            </div>
            <div className="text-[11px] text-muted-foreground border border-border rounded p-3 leading-relaxed">
              <strong>Padrões Toledo:</strong> Prix 4/5 usa 9600 · 8N1 e responde a ENQ (0x05). Prix
              3 transmite continuamente sem requisição — selecione "Prix 3" se sua balança enviar
              peso sem que você aperte nada. Suporta USB→Serial (FTDI/CH340) e serial RS-232 nativo.
              É necessário Chrome/Edge desktop (não funciona em Firefox/Safari).
            </div>
            {reading && (
              <div className="text-xs font-mono border border-border rounded p-2 bg-muted/30">
                Última leitura: {reading.weightKg.toFixed(3)} kg · status {reading.status} · raw "
                {reading.raw}"
              </div>
            )}
          </div>
