...
import { runFiscalRetryPass, getRetryState, resetRetry, type RetryEntry } from "@/lib/fiscal-scheduler";
...
function nextAttemptLabel(entry?: RetryEntry): string {
  if (!entry) return "Na próxima varredura";
  if (entry.permanent) return "Requer correção manual";
  const diff = entry.nextAt - Date.now();
  if (diff <= 0) return "Na próxima varredura";
...
function FiscalErrorsPage() {
  ...
  const retryOne = async (saleId: string) => {
    ...
    try {
      ...
      else toast.error(getRetryState()[saleId]?.lastError ?? "Reemissão falhou novamente");
    } catch (e) {
                ...
                <li key={s.id} className="px-4 py-3 flex flex-col md:flex-row md:items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground mt-1">
                      ...
                      {entry?.attempts ? ` · ${entry.attempts} tentativa(s)` : ""}
                      <span className="inline-flex items-center gap-1 ml-2">
                        <Clock className="size-3" /> {nextAttemptLabel(entry)}
                      </span>
                    </div>
                    {entry?.lastError && (
                      <p className="text-[11px] text-destructive mt-1 break-words font-mono">{entry.lastError}</p>
                    )}
                  </div>
